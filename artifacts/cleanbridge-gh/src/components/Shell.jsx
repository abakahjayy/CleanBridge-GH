...
import {
  ...
  Megaphone, Menu, PackageCheck, Plus, Radio, Route as RouteIcon, ShieldCheck, Truck, UserRound, UsersRound, WalletCards, X
} from 'lucide-react';
...
function Sidebar({ role, isOpen, onClose, unread }) {
  ...
  return <>
    ...
    <aside className={`side-shell ${isOpen ? 'open' : ''}`}>
      <Link href="/" onClick={onClose} aria-label="CleanBridge GH home" data-testid="link-shell-logo"><Logo /></Link>
...
const ROLE_LABEL = { admin: 'Admin', collector: 'Collector', customer: 'Customer' };
function RoleBadge({ role, className = '' }) {
  return <span className={`role-badge role-${role} ${className}`} data-testid={`badge-role-${role}`}>{role === 'admin' && <ShieldCheck size={12} />}{ROLE_LABEL[role]}</span>;
}
...
function ProfileMenu({ user }) {
  ...
  return <div className="profile-menu" ref={ref}>
    ...
    {open && <div className="menu panel" role="menu">
      <div className="menu-head"><Avatar user={user} size={40} /><div><strong>{user.name}</strong><span>{user.email}</span><RoleBadge role={user.role} /></div></div>
    ...
    <main className="app-main">
      <header className="topbar">
        ...
        <div className="topbar-actions">
          {actions}
          {user.role === 'admin' && <RoleBadge role="admin" className="topbar-role" />}
